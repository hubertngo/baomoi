...
#======================Giao Duc=======================================
		...
		begin
		  ...
		  articles = post_list[0].css(".ArticleCateItem")
		  articles[0..-2].each do |post|
		  	begin
		  	...
		  	category.save
		  	post = Article.where(:image => image).first || category.articles.new(title:"#{name}",image:"#{image}",content:"#{content}",desc:"#{desc}",tobao: t)
            post.save
